...
from homepage.models import User, CateringCompany
...
def profile(request):
   ...
   # token=Token.objects.get(user=user.username)
    return render(request, 'company/profile.html', {'user': user, 'company': catering_company})
...
def editprofile(request):
    if request.method == 'POST':
        ...
        company = CateringCompany.objects.get(user_id=user)
